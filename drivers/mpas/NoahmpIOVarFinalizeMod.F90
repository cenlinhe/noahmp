...
! -------------------------------------------------
    ...
    if ( allocated (NoahmpIO%acc_glaflwxy)) deallocate ( NoahmpIO%acc_glaflwxy      )
    ...
    if ( NoahmpIO%IOPT_ALB == 3 ) then
       if ( allocated (NoahmpIO%ss_alb_snw_drc)      ) deallocate ( NoahmpIO%ss_alb_snw_drc      )
       if ( allocated (NoahmpIO%asm_prm_snw_drc)     ) deallocate ( NoahmpIO%asm_prm_snw_drc     )
       if ( allocated (NoahmpIO%ext_cff_mss_snw_drc) ) deallocate ( NoahmpIO%ext_cff_mss_snw_drc )
       if ( allocated (NoahmpIO%ss_alb_snw_dfs)      ) deallocate ( NoahmpIO%ss_alb_snw_dfs      )
       if ( allocated (NoahmpIO%asm_prm_snw_dfs)     ) deallocate ( NoahmpIO%asm_prm_snw_dfs     )
       if ( allocated (NoahmpIO%ext_cff_mss_snw_dfs) ) deallocate ( NoahmpIO%ext_cff_mss_snw_dfs )
       if ( allocated (NoahmpIO%ss_alb_bc1)      )     deallocate ( NoahmpIO%ss_alb_bc1          )
       if ( allocated (NoahmpIO%asm_prm_bc1)     )     deallocate ( NoahmpIO%asm_prm_bc1         )
       if ( allocated (NoahmpIO%ext_cff_mss_bc1) )     deallocate ( NoahmpIO%ext_cff_mss_bc1     )
       if ( allocated (NoahmpIO%ss_alb_bc2)      )     deallocate ( NoahmpIO%ss_alb_bc2          )
       if ( allocated (NoahmpIO%asm_prm_bc2)     )     deallocate ( NoahmpIO%asm_prm_bc2         )
       if ( allocated (NoahmpIO%ext_cff_mss_bc2) )     deallocate ( NoahmpIO%ext_cff_mss_bc2     )
       if ( allocated (NoahmpIO%ss_alb_oc1)      )     deallocate ( NoahmpIO%ss_alb_oc1          )
       if ( allocated (NoahmpIO%asm_prm_oc1)     )     deallocate ( NoahmpIO%asm_prm_oc1         )
       if ( allocated (NoahmpIO%ext_cff_mss_oc1) )     deallocate ( NoahmpIO%ext_cff_mss_oc1     )
       if ( allocated (NoahmpIO%ss_alb_oc2)      )     deallocate ( NoahmpIO%ss_alb_oc2          )
       if ( allocated (NoahmpIO%asm_prm_oc2)     )     deallocate ( NoahmpIO%asm_prm_oc2         )
       if ( allocated (NoahmpIO%ext_cff_mss_oc2) )     deallocate ( NoahmpIO%ext_cff_mss_oc2     )
       if ( allocated (NoahmpIO%ss_alb_dst1)     )     deallocate ( NoahmpIO%ss_alb_dst1         )
       if ( allocated (NoahmpIO%asm_prm_dst1)    )     deallocate ( NoahmpIO%asm_prm_dst1        )
       if ( allocated (NoahmpIO%ext_cff_mss_dst1))     deallocate ( NoahmpIO%ext_cff_mss_dst1    )
       if ( allocated (NoahmpIO%ss_alb_dst2)     )     deallocate ( NoahmpIO%ss_alb_dst2         )
       if ( allocated (NoahmpIO%asm_prm_dst2)    )     deallocate ( NoahmpIO%asm_prm_dst2        )
       if ( allocated (NoahmpIO%ext_cff_mss_dst2))     deallocate ( NoahmpIO%ext_cff_mss_dst2    )
       if ( allocated (NoahmpIO%ss_alb_dst3)     )     deallocate ( NoahmpIO%ss_alb_dst3         )
       if ( allocated (NoahmpIO%asm_prm_dst3)    )     deallocate ( NoahmpIO%asm_prm_dst3        )
       if ( allocated (NoahmpIO%ext_cff_mss_dst3))     deallocate ( NoahmpIO%ext_cff_mss_dst3    )
       if ( allocated (NoahmpIO%ss_alb_dst4)     )     deallocate ( NoahmpIO%ss_alb_dst4         )
       if ( allocated (NoahmpIO%asm_prm_dst4)    )     deallocate ( NoahmpIO%asm_prm_dst4        )
       if ( allocated (NoahmpIO%ext_cff_mss_dst4))     deallocate ( NoahmpIO%ext_cff_mss_dst4    )
       if ( allocated (NoahmpIO%ss_alb_dst5)     )     deallocate ( NoahmpIO%ss_alb_dst5         )
       if ( allocated (NoahmpIO%asm_prm_dst5)    )     deallocate ( NoahmpIO%asm_prm_dst5        )
       if ( allocated (NoahmpIO%ext_cff_mss_dst5))     deallocate ( NoahmpIO%ext_cff_mss_dst5    )
       if ( allocated (NoahmpIO%flx_wgt_dir)     )     deallocate ( NoahmpIO%flx_wgt_dir         )
       if ( allocated (NoahmpIO%flx_wgt_dif)     )     deallocate ( NoahmpIO%flx_wgt_dif         )
       if ( allocated (NoahmpIO%snowage_tau)     )     deallocate ( NoahmpIO%snowage_tau         )
       if ( allocated (NoahmpIO%snowage_kappa)   )     deallocate ( NoahmpIO%snowage_kappa       )
       if ( allocated (NoahmpIO%snowage_drdt0)   )     deallocate ( NoahmpIO%snowage_drdt0       )
       if ( allocated (NoahmpIO%SNRDSXY) )             deallocate ( NoahmpIO%SNRDSXY             )
       if ( allocated (NoahmpIO%SNFRXY)  )             deallocate ( NoahmpIO%SNFRXY              )
       if ( allocated (NoahmpIO%BCPHIXY) )             deallocate ( NoahmpIO%BCPHIXY             )
       if ( allocated (NoahmpIO%BCPHOXY) )             deallocate ( NoahmpIO%BCPHOXY             )
       if ( allocated (NoahmpIO%OCPHIXY) )             deallocate ( NoahmpIO%OCPHIXY             )
       if ( allocated (NoahmpIO%OCPHOXY) )             deallocate ( NoahmpIO%OCPHOXY             )
       if ( allocated (NoahmpIO%DUST1XY) )             deallocate ( NoahmpIO%DUST1XY             )
       if ( allocated (NoahmpIO%DUST2XY) )             deallocate ( NoahmpIO%DUST2XY             )
       if ( allocated (NoahmpIO%DUST3XY) )             deallocate ( NoahmpIO%DUST3XY             )
       if ( allocated (NoahmpIO%DUST4XY) )             deallocate ( NoahmpIO%DUST4XY             )
       if ( allocated (NoahmpIO%DUST5XY) )             deallocate ( NoahmpIO%DUST5XY             )
       if ( allocated (NoahmpIO%MassConcBCPHIXY) )     deallocate ( NoahmpIO%MassConcBCPHIXY     )
       if ( allocated (NoahmpIO%MassConcBCPHOXY) )     deallocate ( NoahmpIO%MassConcBCPHOXY     )
       if ( allocated (NoahmpIO%MassConcOCPHIXY) )     deallocate ( NoahmpIO%MassConcOCPHIXY     )
       if ( allocated (NoahmpIO%MassConcOCPHOXY) )     deallocate ( NoahmpIO%MassConcOCPHOXY     )
       if ( allocated (NoahmpIO%MassConcDUST1XY) )     deallocate ( NoahmpIO%MassConcDUST1XY     )
       if ( allocated (NoahmpIO%MassConcDUST2XY) )     deallocate ( NoahmpIO%MassConcDUST2XY     )
       if ( allocated (NoahmpIO%MassConcDUST3XY) )     deallocate ( NoahmpIO%MassConcDUST3XY     )
       if ( allocated (NoahmpIO%MassConcDUST4XY) )     deallocate ( NoahmpIO%MassConcDUST4XY     )
       if ( allocated (NoahmpIO%MassConcDUST5XY) )     deallocate ( NoahmpIO%MassConcDUST5XY     )
       if ( allocated (NoahmpIO%DepBChydrophoXY) )     deallocate ( NoahmpIO%DepBChydrophoXY     )
       if ( allocated (NoahmpIO%DepBChydrophiXY) )     deallocate ( NoahmpIO%DepBChydrophiXY     )
       if ( allocated (NoahmpIO%DepOChydrophoXY) )     deallocate ( NoahmpIO%DepOChydrophoXY     )
       if ( allocated (NoahmpIO%DepOChydrophiXY) )     deallocate ( NoahmpIO%DepOChydrophiXY     )
       if ( allocated (NoahmpIO%DepDust1XY)      )     deallocate ( NoahmpIO%DepDust1XY          )
       if ( allocated (NoahmpIO%DepDust2XY)      )     deallocate ( NoahmpIO%DepDust2XY          )
       if ( allocated (NoahmpIO%DepDust3XY)      )     deallocate ( NoahmpIO%DepDust3XY          )
       if ( allocated (NoahmpIO%DepDust4XY)      )     deallocate ( NoahmpIO%DepDust4XY          )
       if ( allocated (NoahmpIO%DepDust5XY)      )     deallocate ( NoahmpIO%DepDust5XY          )
    endif

    if ( allocated (NoahmpIO%ALBSNOWDIRXY) ) deallocate ( NoahmpIO%ALBSNOWDIRXY ) ! snow albedo (direct)
    if ( allocated (NoahmpIO%ALBSNOWDIFXY) ) deallocate ( NoahmpIO%ALBSNOWDIFXY ) ! snow albedo (diffuse)
    if ( allocated (NoahmpIO%ALBSFCDIRXY)  ) deallocate ( NoahmpIO%ALBSFCDIRXY  ) ! surface albedo (direct)
    if ( allocated (NoahmpIO%ALBSFCDIFXY)  ) deallocate ( NoahmpIO%ALBSFCDIFXY  ) ! surface albedo (diffuse)
    if ( allocated (NoahmpIO%ALBSOILDIRXY) ) deallocate ( NoahmpIO%ALBSOILDIRXY ) ! soil albedo (direct)
    if ( allocated (NoahmpIO%ALBSOILDIFXY) ) deallocate ( NoahmpIO%ALBSOILDIFXY ) ! soil albedo (diffuse)
    if ( allocated (NoahmpIO%RadSwVisFrac) ) deallocate ( NoahmpIO%RadSwVisFrac ) ! downward solar radation visible fraction
    if ( allocated (NoahmpIO%RadSwDirFrac) ) deallocate ( NoahmpIO%RadSwDirFrac ) ! downward solar radation direct fraction
    ...
    if ( NoahmpIO%IOPT_WETLAND > 0 ) then
       if ( allocated (NoahmpIO%FSATXY) ) deallocate ( NoahmpIO%FSATXY  ) ! saturated fraction of the grid (-)
       if ( allocated (NoahmpIO%WSURFXY)) deallocate ( NoahmpIO%WSURFXY ) ! wetland water storage [mm]
    endif
    if ( NoahmpIO%IOPT_WETLAND == 2 ) then
       if ( allocated (NoahmpIO%FSATMX) ) deallocate ( NoahmpIO%FSATMX  ) ! maximum saturated fraction
       if ( allocated (NoahmpIO%WCAP)   ) deallocate ( NoahmpIO%WCAP    ) ! maximum wetland capacity [m]
    endif
